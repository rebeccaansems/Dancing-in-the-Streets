    ...
    (function() {
        ...
        MmJsBridge.vpaid = {
            init: function(options) {
                ...
                var secondPartOfScript = "ipt";
                iframe.contentWindow.document.write("<scr" + secondPartOfScript + ' src="' + $parsedVast.vpaidUrl + '"></scr' + secondPartOfScript + ">");
                iframe.contentWindow.document.close();
                var timesCheckedForReady = 0;
                var intervalTimingMs = 50;
                var maxIntervalTries = Math.ceil(options.adUnitTimeout / intervalTimingMs);
                var intervalStartTime = Date.now();
                var waitForVpaidReadyInterval = setInterval(function() {
                    timesCheckedForReady++;
                    if (iframe.contentWindow.getVPAIDAd) {
                        log.debug("Found iframe.contentWindow.getVPAIDAd");
                        clearInterval(waitForVpaidReadyInterval);
                    ...
                    } else if (timesCheckedForReady > maxIntervalTries || timesCheckedForReady % 10 === 0 && Date.now() - intervalStartTime > options.adUnitTimeout) {
                        clearInterval(waitForVpaidReadyInterval);
        ...
        function parseCompanionAds(companionAdsElement) {
            ...
            if (companionElement) {
                ...
                if (resourceType == "static") {
                    ...
                    xhttp.onreadystatechange = function() {
                        if (xhttp.readyState == 4) {
                            if (xhttp.status < 400) {
                                ...
                                companionJson.html = xhttp.responseText;
                            } else {
                    ...
                    xhttp.open("GET", resourceElement.textContent.trim(), true);
                    xhttp.send();
